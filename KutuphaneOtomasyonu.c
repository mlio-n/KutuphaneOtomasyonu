...
// fonksiyonlar içersinde kullanılması gereken fonksiyonlar.
...
void dosya_kontrol(FILE *dosya, const char *dosya_adi) {
    if (dosya == NULL) {
        printf("Hata: %s dosyasi olusturulamadi veya acilamadi.\n", dosya_adi);
        exit(EXIT_FAILURE);
    }
}
...
void odunc_al() {
    ...
    if (odunc_alinan_kitap_var_mi(kullanici_numara)) {
        printf("Bu ogrenci zaten bir kitap odunc almis durumda.\n");
        return;
    }
...
int kitap_odunc_alindi_mi(int kitap_numara) {
    ...
    if (dosya == NULL) {
        printf("Hata: Odunc dosyasi acilamadi.\n");
        return 0;
    }

    struct Odunc odunc;

    while (fscanf(dosya, "%d %d %*s\n", &odunc.kullanici_numara, &odunc.kitap_numara) != EOF) {
